...
import { setToken, setAdminType } from "../main"

export function LoginPage() {
  const navigate = useNavigate()
  const { t, locale, setLocale } = useI18n()
  const { theme, toggleTheme } = useTheme()
  const [tab, setTab] = useState<"owner" | "store">("owner")
    ...
    <div className="min-h-dvh flex items-center justify-center relative overflow-hidden bg-slate-950">
      ...
      <div className="relative z-10 w-full max-w-md mx-4 animate-fade-in">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white tracking-tight">Lebanon POS</h1>
          <p className="text-sm text-indigo-300/70 mt-1">Admin Panel</p>
        </div>

        <div className="flex mb-6 rounded-xl bg-white/[0.04] border border-white/[0.06] p-1">
          <button onClick={() => { setTab("owner"); setError("") }}
            className={`flex-1 px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 ${tab === "owner" ? "bg-gradient-to-r from-indigo-600 to-violet-600 text-white shadow-lg shadow-indigo-600/20" : "text-indigo-300/60 hover:text-white"}`}>
            Owner Login
          </button>
          <button onClick={() => { setTab("store"); setError("") }}
            className={`flex-1 px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 ${tab === "store" ? "bg-gradient-to-r from-indigo-600 to-violet-600 text-white shadow-lg shadow-indigo-600/20" : "text-indigo-300/60 hover:text-white"}`}>
            Store Login
          </button>
        </div>

        {tab === "owner" ? (
          <form onSubmit={handleOwnerLogin} className="backdrop-blur-xl bg-white/[0.04] border border-white/[0.06] rounded-2xl p-8 shadow-2xl">
            {error && (
          ...
          <form onSubmit={handleStoreLogin} className="backdrop-blur-xl bg-white/[0.04] border border-white/[0.06] rounded-2xl p-8 shadow-2xl">
            {error && (
              <div className="mb-5 p-3.5 bg-rose-500/10 border border-rose-500/20 text-rose-300 text-sm rounded-xl flex items-center gap-2 animate-slide-up">
                <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                {error}

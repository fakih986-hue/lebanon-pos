...
const OrderCard = memo(function OrderCard({
  ...
  drivers: Driver[]
  driverSearch: Record<string, string>
  showDropdown: Record<string, boolean>
  onToggleExpand: (id: string) => void
  onAssignDriver: (orderId: string, driverId: string) => void
  onUpdateStatus: (id: string, status: string) => void
  onCancel: (id: string) => void
  onDriverSearchChange: (orderId: string, value: string) => void
  onDriverFocus: (orderId: string) => void
  onDriverBlur: (orderId: string) => void
  searchRefs: React.MutableRefObject<Record<string, HTMLDivElement | null>>
  t: (k: string) => string
}) {
  ...
  const filteredDrivers = useMemo(() =>
    drivers.filter(d => !q || d.name.toLowerCase().includes(q) || d.code?.toLowerCase().includes(q)),
  [drivers, q])

  return (
    <div className="data-card p-0 overflow-hidden animate-slide-up">
      <div className="p-5 cursor-pointer" onClick={() => onToggleExpand(order.id)}>
        <div className="flex items-center gap-4">
          ...
          <svg className={`w-5 h-5 transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`} style={{ color: "var(--text-muted)" }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {isExpanded && (
        <div className="border-t px-5 py-4 space-y-4 animate-slide-up" style={{ borderColor: "var(--border-card)", background: "var(--row-hover)" }}>
          ...
          <div className="flex items-center gap-2 text-sm flex-wrap">
            ...
            {order.customerPhone && (
              <a href={`https://wa.me/${order.customerPhone.replace(/^0+/, "961")}?text=${encodeURIComponent(`Hi ${order.customerName}, your order ${order.orderNumber} status: ${order.status}`)}`}
            ...
            {order.status !== "Delivered" && order.status !== "Cancelled" && (
              <>
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative" ref={el => { if (el) searchRefs.current[order.id] = el }}>
                    <input value={driverSearch[order.id] ?? (order.assignedName || "")}
                      onChange={e => onDriverSearchChange(order.id, e.target.value)}
                      onFocus={() => onDriverFocus(order.id)}
                      onBlur={() => onDriverBlur(order.id)}
                ...
                {nextSt && (
                  <button onClick={() => onUpdateStatus(order.id, nextSt)}
                    className="btn-primary text-xs py-2 px-4">
                    {t("delivery.mark_as")} {t(`status.${nextSt}`)}
                  </button>
                )}
...
export function DeliveryPage() {
  ...
  useEffect(() => { load(); loadDrivers(); fetchOnlineDrivers() }, [load, loadDrivers, fetchOnlineDrivers])
  useEffect(() => {
  ...
  const handleDriverSearchChange = useCallback((orderId: string, value: string) => {
    setDriverSearch(p => ({ ...p, [orderId]: value }))
    setShowDropdown(p => ({ ...p, [orderId]: true }))
  }, [])

  const handleDriverFocus = useCallback((orderId: string) => {
    setShowDropdown(p => ({ ...p, [orderId]: true }))
  }, [])

  const handleDriverBlur = useCallback((orderId: string) => {
    setTimeout(() => setShowDropdown(p => ({ ...p, [orderId]: false })), 200)
  }, [])
  ...
  const orderCardProps = useMemo(() => ({
    ...
    t,
  }), [onlineDrivers, drivers, driverSearch, showDropdown, t])
    ...
    <div className="space-y-6 pb-12">
      <div className="flex items-center justify-between flex-wrap gap-4 animate-fade-in">
        ...
        <div className="flex gap-2 items-center">
          ...
          <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value); setPage(1) }} className="select-field text-xs py-2 w-32">
        ...
        <div className="space-y-3">
          {orders.map((order) => (
            <OrderCard
              ...
              onlineDrivers={orderCardProps.onlineDrivers}
              drivers={orderCardProps.drivers}
              driverSearch={orderCardProps.driverSearch}
              showDropdown={orderCardProps.showDropdown}
              ...
              onDriverBlur={handleDriverBlur}
              searchRefs={orderCardProps.searchRefs}
              t={orderCardProps.t}
            />
          ))}
      ...
      {confirmCancelId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setConfirmCancelId(null)}>
          <div className="rounded-2xl border p-6 w-full max-w-sm mx-4 shadow-2xl" onClick={e => e.stopPropagation()}
            ...
            <h3 className="text-sm font-bold mb-2" style={{ color: "var(--text-primary)" }}>{t("delivery.cancel_order")}</h3>
            <p className="text-xs mb-4" style={{ color: "var(--text-secondary)" }}>{t("admin.cancel_confirm")}</p>
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={() => { updateStatus(confirmCancelId, "Cancelled"); setConfirmCancelId(null) }}
                className="px-4 py-2 rounded-xl bg-rose-600 text-white text-sm font-semibold hover:bg-rose-500 transition-all">
                {t("delivery.cancel_order")}
              </button>
            </div>
          </div>

      ...
      <div className="max-w-lg mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <button onClick={() => navigate(`/order/${tenantSubdomain}`)}
            className="text-sm text-secondary hover:text-primary">&larr; Menu</button>
          ...
          <div className="space-y-3">
            {orders.map(order => (
              <div key={order.id} className="bg-glass border border-glass rounded-xl p-4">
                ...
                <p className="text-sm text-secondary">{order.items?.length || 0} items &middot; ${order.total?.toFixed(2)}</p>
